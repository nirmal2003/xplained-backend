...
public interface AnimationRepository extends JpaRepository<Animation, Long> {
    ...
    @Query("SELECT new com.xplained.main.dto.models.objects.animations.AnimationResponse(a.id, a.objectId, a.type, a.start, a.finish, a.duration, a.angle, a.xAxis, a.yAxis, a.createdAt) FROM Animation a WHERE a.objectId = :objectId ORDER BY a.createdAt DESC")
    List<AnimationResponse> findAllByObjectIdOrderByCreatedAt(@Param("objectId") Long objectId, Pageable pageable);

    @Query("SELECT new com.xplained.main.dto.models.objects.animations.AnimationResponse(a.id, a.objectId, a.type, a.start, a.finish, a.duration, a.angle, a.xAxis, a.yAxis, a.createdAt) FROM Animation a WHERE a.modelId = :modelId ORDER BY a.createdAt DESC")
