...
//            ", r.createdAt) FROM LessonResources r JOIN User u ON u.id = :userId WHERE r.lessonId = :lessonId ORDER BY r.createdAt ASC")
    @Query("SELECT new com.xplained.main.dto.courses.modules.lessons.resources.LessonResourcesResponse(r.id, r.type, r.name, " +
            ...
            ", CASE WHEN EXISTS (SELECT cr FROM CompletedResources cr WHERE cr.resourcesId = r.id AND cr.userId = :userId) THEN true ELSE false END, r.createdAt) FROM LessonResources r JOIN User u ON u.id = :userId WHERE r.lessonId = :lessonId ORDER BY r.createdAt ASC")
